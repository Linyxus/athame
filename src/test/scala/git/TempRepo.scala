...
private[git] object TestFs extends js.Object:
...
@js.native
private[git] trait TestStats extends js.Object:
  def mtimeMs: Double = js.native

@js.native
@JSImport("node:os", JSImport.Namespace)
private[git] object TestOs extends js.Object:
  def tmpdir(): String = js.native
  def platform(): String = js.native

@js.native
@JSImport("node:path", JSImport.Namespace)
private[git] object TestPath extends js.Object:
...
/** A throwaway git repository living under the OS temp directory.
  ...
  * able to reach the repository athame itself lives in, so tests never name a repository any other
  * way.
  *
...
object TempRepo:
  def create(): TempRepo =
    val dir = TestFs.mkdtempSync(TestPath.join(TestOs.tmpdir(), "ame-test-"))
