...
/** Behavior tests for the snapshot primitive, organized by the rules they pin (S1-S10).
  ...
  * repository any other way, so nothing in this suite can reach athame's own git state.
  */
class SnapshotSuite extends munit.FunSuite:
  ...
  test("create: the ref lands in the hidden namespace at the returned commit"):
    withRepo: repo =>
      ...
      assertEquals(result.ref, "refs/ame/snapshots/s1")
  ...
  test("create: the commit message names the snapshot"):
    withRepo: repo =>
      seed(repo)
      val result = snapshot(repo, "s1")
      assertEquals(repo.git("log", "-1", "--format=%s", result.commit), "ame snapshot s1")
  ...
  test("identity: the author is the tool"):
    withRepo: repo =>
      seed(repo)
      val result = snapshot(repo, "s1")
      assertEquals(repo.git("log", "-1", "--format=%an <%ae>", result.commit), "ame <ame@snapshot>")

  test("identity: the committer is the tool"):
    withRepo: repo =>
      seed(repo)
      val result = snapshot(repo, "s1")
      assertEquals(repo.git("log", "-1", "--format=%cn <%ce>", result.commit), "ame <ame@snapshot>")
    ...
    withoutAmbientGitConfig:
      withRepo: repo =>
        ...
        assertEquals(
          ...
          "ame <ame@snapshot> ame <ame@snapshot>"
        )
  ...
  test("ids: slashes are legal, so an id can carry structure"):
    withRepo: repo =>
      ...
      assertEquals(result.ref, "refs/ame/snapshots/sync/1/before")
  ...
  test("delete: the ref goes away and the listing shrinks"):
    withRepo: repo =>
      ...
      assert(repo.tryGit("rev-parse", "--verify", "refs/ame/snapshots/s1").isLeft)
